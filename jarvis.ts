...
function detectActivityIntent(input: string): "handover" | "scope" | "risk" | "progress" | "initialize" | "direction" | "default" {
...
function activityStepsForIntent(intent: ReturnType<typeof detectActivityIntent>): ActivityStep[] {
  const steps: Record<ReturnType<typeof detectActivityIntent>, ActivityStep[]> = {
    initialize: [
...
function JarvisActivityPanel({ intent }: { intent: ReturnType<typeof detectActivityIntent> }) {
...
export default function Home() {
  ...
  const [activityIntent, setActivityIntent] = useState<ReturnType<typeof detectActivityIntent>>("default");
