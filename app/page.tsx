...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      const normalized = normalizeProjectState(parsed, defaultProjectState);
      setProject(normalized);
      setDraftProject(normalized);
      if (!savedHistory && normalized.mission) fetchOrientation(normalized, loadedPassword);
    }
    if (savedHistory) {
      ...
      if (parsedHistory.length > 1) {
        ...
        if (last.timestamp) {
          ...
          if (ageHours >= 8) {
            const savedProject = saved ? normalizeProjectState(JSON.parse(saved), defaultProjectState) : null;
  ...
  function projectStateMarkdown(state: ProjectState) {
    return `# PROJECT_STATE\n\nGenerated by Jarvis.\n\n## Mission\n\n${state.mission || "Not initialized"}\n\n## Status\n\n${state.status || "Unknown"}\n\n## Confidence\n\n${state.confidence || "Unknown"}\n\n## Approval\n\n${state.approval || "Not established"}\n\n## Progress\n\n${state.progress || 0}%\n\n## Next Action\n\n${state.nextAction || "Not defined"}\n\n## Risks\n\n${state.risks.length ? state.risks.map((r) => `- ${r}`).join("\n") : "- None identified"}\n\n## Decisions\n\n${state.decisions.length ? state.decisions.map((d) => {
      let out = `- ${d.text}`;
      if (d.reasoning) out += `\n  - Why: ${d.reasoning}`;
      if (d.alternatives) out += `\n  - Considered: ${d.alternatives}`;
        ...
        >
          {editMode ? (
            <div className="space-y-3">
              ...
              <div>
                <label className="mb-1 block text-[10px] font-semibold uppercase tracking-widest text-gray-400">Decisions</label>
                {draftProject.decisions.map((d, i) => (
                  <div key={d.id} className="mb-3 rounded border border-gray-200 p-2.5 space-y-1.5">
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-[10px] text-gray-400 font-medium uppercase tracking-wider">Decision {i + 1}</span>
                      <button onClick={() => setDraftProject(p => ({ ...p, decisions: p.decisions.filter((_, idx) => idx !== i) }))} className="text-[10px] text-gray-300 hover:text-red-400">✕</button>
                    </div>
                    <textarea rows={2} value={d.text} placeholder="Decision text" onChange={e => setDraftProject(p => ({ ...p, decisions: p.decisions.map((x, idx) => idx === i ? { ...x, text: e.target.value } : x) }))} className="w-full resize-none rounded border border-gray-200 px-2 py-1 text-xs text-gray-800 outline-none focus:border-gray-400 placeholder:text-gray-300" />
                    <textarea rows={1} value={d.reasoning || ""} placeholder="Why this was chosen (optional)" onChange={e => setDraftProject(p => ({ ...p, decisions: p.decisions.map((x, idx) => idx === i ? { ...x, reasoning: e.target.value || undefined } : x) }))} className="w-full resize-none rounded border border-gray-200 px-2 py-1 text-xs text-gray-700 outline-none focus:border-gray-400 placeholder:text-gray-300" />
                    <textarea rows={1} value={d.alternatives || ""} placeholder="Alternatives considered (optional)" onChange={e => setDraftProject(p => ({ ...p, decisions: p.decisions.map((x, idx) => idx === i ? { ...x, alternatives: e.target.value || undefined } : x) }))} className="w-full resize-none rounded border border-gray-200 px-2 py-1 text-xs text-gray-700 outline-none focus:border-gray-400 placeholder:text-gray-300" />
                    <div className="flex items-center gap-2">
                      <select value={d.confidence} onChange={e => setDraftProject(p => ({ ...p, decisions: p.decisions.map((x, idx) => idx === i ? { ...x, confidence: e.target.value as Decision["confidence"] } : x) }))} className="rounded border border-gray-200 px-1.5 py-0.5 text-xs text-gray-600 outline-none">
                ...
                {draftProject.decisions.length < 5 && (
                  <button onClick={() => setDraftProject(p => ({ ...p, decisions: [...p.decisions, { id: `manual-${Date.now()}`, text: "", confidence: "medium", createdAt: new Date().toISOString(), source: "user" }] }))} className="w-full rounded border border-dashed border-gray-200 py-1.5 text-[11px] text-gray-400 hover:border-gray-300 hover:text-gray-600">
                    + Add decision
                  </button>
                )}
              </div>
